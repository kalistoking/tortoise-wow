...
# boost/algorithm/string.hpp, boost/bimap.hpp (+ bimap/multiset_of.hpp), and
# boost/stacktrace.hpp (MemoryMonitor.cpp, gated behind BOOST_STACKTRACE_LINK). Several other
...
if (-not (Test-Path $SourceDir)) {
    ...
    # Refuse to move a branch the remote owns over uncommitted work, rather than letting a
    # hard reset silently take it. Checked unconditionally here, not just on a branch switch:
    # $BranchName reused across runs is normally already checked out, and content sitting
    # there uncommitted is exactly what "reset --hard" below would discard without asking.
    # A branch with no remote counterpart is a different case entirely - see below - and is
    # not touched by this.
    if ($BranchIsOnRemote) {
        $DirtyFiles = @(git status --porcelain --untracked-files=no)
        if ($DirtyFiles.Count -gt 0) {
            Pop-Location
            Stop-Pipeline -Message ("$SourceDir has uncommitted changes to tracked files, and '$BranchName' exists " +
                                    "on $RepoUrl - this script always makes such a branch match that remote exactly, " +
                                    "which a reset would take these with it:`n  " +
                                    (($DirtyFiles | Select-Object -First 10) -join "`n  ") +
                                    "`nCommit or stash them first, or point -RepoUrl/-BranchName somewhere this " +
                                    "checkout is not also being used for other work.")
        }
    }
    ...
    if ($BranchIsOnRemote) {
        # Reset rather than pull, unconditionally - not just when this happens to already
        # match. A branch that exists on -RepoUrl always ends this step looking exactly like
        # $SourceRemote/$BranchName; nothing here is a place to accumulate local history, so
        # there is nothing to reconcile and nothing a merge or rebase needs to attempt.
        #
        # This is what -applyPatches commits onto every run, without ever being asked to
        # clean up after itself - by design, so a run that fails partway through still leaves
        # a checkout someone can inspect. A second run used to have to choose between a pull
        # that conflicted the moment the remote's own eventual fix read even slightly
        # differently from what was cherry-picked here (reviewed wording, a trimmed comment -
        # exactly what happened to this script's own patches), and a rebase that turned out to
        # have the identical problem (patch-id compares the exact diff, not the fix). Neither
        # extra care is needed once the branch is simply put back to what the remote says it
        # is before anything gets cherry-picked onto it again.
        Invoke-NativeLogged -Executable "git" -Arguments @("reset", "--hard", "$SourceRemote/$BranchName")
        Assert-LastExitCode -Message "git reset of '$BranchName' to $SourceRemote/$BranchName failed"
    } else {
...
# External dependencies from the vcpkg toolchain - not all of it required the same way.
$VcpkgBinDir = Join-Path $VcpkgInstalledPath "bin"

# ACE.dll: the one genuinely unconditional entry here, for every core branch this pipeline
# can build. Confirmed with dumpbin /dependents, and by actually running an installed build
# with it missing - STATUS_DLL_NOT_FOUND at startup, immediately, on both mangosd.exe and
# realmd.exe. Nothing in CMakeLists.txt's own install() step deploys it (ACE is never
# bundled under dep/windows the way OpenSSL/MySQL are), so this pipeline has to.
$AceDllPath = Join-Path $VcpkgBinDir "ACE.dll"
if (-not (Test-Path -LiteralPath $AceDllPath)) {
    Stop-Pipeline -Message "Required runtime dependency is missing from the vcpkg toolchain: $AceDllPath"
}
Copy-Item -Path $AceDllPath -Destination $LibDir -Force

# libcrypto-3-x64.dll/libssl-3-x64.dll: deliberately NOT deployed, on either core branch this
# pipeline is actually tracking:
#   - the engine's own include-order issue, tortoise-wow/tortoise-wow#496, makes a core-only
#     build need neither file at all once fixed (confirmed with dumpbin /dependents against a
#     real build: no -3-x64 DLL in mangosd.exe's or realmd.exe's import table);
#   - a -WithPlayerBots build needed one fix on top of that - RAND_bytes, called from
#     TortoiseBots' own RandomBotService.cpp, traced with dumpbin /imports down to the exact
#     symbol and fixed on the module side in Sagiroth/TortoiseBots#170 - and with both fixes
#     applied together needs neither file either (confirmed the same way: dumpbin /imports
#     against a real -WithPlayerBots build with both applied).
# Every branch and patch this pipeline currently builds against carries both fixes, so this
# pair is dead weight, not a safety net, for any of them - copying it defensively just made a
# missing DLL harder to notice on a checkout that had actually regressed. If a future branch
# genuinely needs this pair again (missing either fix, or a module that reintroduces a real
# RAND_bytes/ERR_* call - the include-order fix in modules/CMakeLists.txt on this branch
# still only helps if the library actually exports what gets called, see that file's own
# comment on this), the fix belongs in the checkout itself, the same way #496 and
...
# boost_*.dll: best-effort too, and in every build tested so far, not actually needed at
# all - confirmed with dumpbin /dependents against a full -WithPlayerBots build. TortoiseBots'
# own boost::stacktrace usage (the only Boost header linked here with a compiled backend,
# behind BOOST_STACKTRACE_LINK in MemoryMonitor.cpp) is currently unreachable from anything
# the linker keeps, so no boost_*.dll ends up in the final binaries' import table - but that
# depends on what the module's source does, not on anything this pipeline controls, so this
# copies whatever vcpkg produced rather than assume there is nothing to copy.
